...
15. **Naive-timestamp query params (post-launch bugfix)** — TypeORM's `created/updatedAt` columns are naive `timestamp` holding UTC wall time, but the pg driver serializes JS `Date` params as *local* wall time; on a UTC+3 host every `updatedSince` delta ran 3 hours in the future and returned nothing (boards/tracking only updated on reload). All cursor and hotel-day-window comparisons against naive columns now go through `naiveUtc()` (`stay-time.ts`), which passes UTC ISO strings — host-timezone-independent. Applies to the requests board too. `timestamptz` columns (`dueAt`, `deliveredAt`, …) are unaffected. Guest order view exposes `settled` so the app can show the room-bill balance.
