...
#   ./dev.sh --no-migrate    skip `migration:run` on boot
#   ./dev.sh backend admin   run only the named services
#   ./dev.sh --install       npm install anywhere node_modules is missing
#
...
RUN_DB=1 RUN_MIGRATE=1 DO_INSTALL=0
SERVICES=()
for arg in "$@"; do
  case "$arg" in
    --no-db)      RUN_DB=0 ;;
    ...
    backend|admin|tenant) SERVICES+=("$arg") ;;
...
if wants backend; then
  ...
  if [[ $RUN_DB -eq 1 ]]; then
    ...
    if [[ $RUN_MIGRATE -eq 1 ]]; then
      ...
      if (cd "$BACKEND" && npm run migration:run) >/tmp/gxp-migrate.log 2>&1; then
        ok "schema up to date"
      else
        warn "migration:run failed — see /tmp/gxp-migrate.log (continuing)"
      fi
    fi
  else
...
wants tenant  && ok "tenant   → http://localhost:3001  (or sunrise.lvh.me:3001)"
